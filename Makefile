...
SGX_MODE ?= HW
SGX_ARCH ?= x64
SGX_SIGN_PEM ?= enclave/Enclave_private.pem
COMMON_PATH := ./rust-sgx-sdk
...
ifeq ($(SGX_DEBUG), 1)
	...
	SGX_ENCLAVE_CONFIG = enclave/Enclave.config.prod.xml
endif
...
CUSTOM_BIN_PATH := ./bin
CUSTOM_EDL_PATH := $(COMMON_PATH)/edl
CUSTOM_COMMON_PATH := $(COMMON_PATH)/common
...
App_Rust_Path := target/$(App_Rust_Mode)
...
$(Signed_RustEnclave_Name): $(RustEnclave_Name)
	mkdir -p bin
	@echo "SIGN_PEM => $(SGX_SIGN_PEM)"
	@$(SGX_ENCLAVE_SIGNER) sign -key $(SGX_SIGN_PEM) -enclave $(RustEnclave_Name) -out $@ -config $(SGX_ENCLAVE_CONFIG) 
	@echo "SIGN =>  $@"
